  ...
  try {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:5001/api/budgets/${budgetId}`,
        {
          method: "GET",
          headers: {
      ...
      if (!response.ok) {
        ...
        return NextResponse.json(
          { error: errorData.error || "Failed to fetch budget" },
          { status: response.status }
        );
      }
...
export async function PUT(
  ...
  try {
    ...
    console.log(`Updating budget ${budgetId} with data:`, body);
    ...
    try {
      ...
      if (!response.ok) {
        ...
        return NextResponse.json(
          { error: errorData.error || "Failed to update budget" },
          { status: response.status }
        );
      }
  ...
  try {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        return NextResponse.json(
          { error: errorData.error || "Failed to delete budget" },
          { status: response.status }
        );
      }
